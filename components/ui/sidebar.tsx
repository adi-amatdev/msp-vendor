...
>(
  (
    {
      defaultOpen: defaultOpenOld = true,
      open: openProp,
    ...
    const [_open, _setOpen] = React.useState(defaultOpenOld)
    ...
    const toggleSidebarOld = React.useCallback(() => {
    ...
    const contextValue = React.useMemo<SidebarContextProps>(
      () => ({
        isOpen: open,
        toggleSidebar: toggleSidebarOld,
        openSidebar: () => setOpen(true),
        closeSidebar: () => setOpen(false),
      }),
      [open, toggleSidebarOld, setOpen],
    )
...
export function Sidebar({ className, ...props }: SidebarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed inset-y-0 left-0 z-20 w-64 transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0",
...
export function SidebarTrigger({ className, ...props }: SidebarTriggerProps) {
  ...
  return (
    <button type="button" onClick={toggleSidebar} className={cn("p-2 rounded-md", className)} {...props}>
      <svg
...
export function SidebarMenuButton({ className, asChild = false, ...props }: SidebarMenuButtonProps) {
  if (asChild) {
    // When asChild is true, we don't render any wrapper element
    // and just pass the children through
    return <>{props.children}</>
  }

  // When asChild is false, render a normal button with the className
  return (
    <button
      className={cn(
        "w-full rounded-md px-3 py-2 text-left text-sm font-medium transition-colors hover:bg-gray-100 hover:text-gray-900 focus:bg-gray-100 focus:text-gray-900 focus:outline-none dark:hover:bg-gray-800 dark:hover:text-gray-50 dark:focus:bg-gray-800 dark:focus:text-gray-50",
        className,
      )}
      {...props}
    />
